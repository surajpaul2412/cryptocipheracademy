...
<div class="table-responsive px-3 pb-5">
 <table class="table table-striped">
    ...
    <tbody>
      @foreach($aboutUsLibraryImage as $index => $new)
      <tr>
        <th>{{$index+1}}.</th>
        <td><img src="{{env('image_url')}}/aboutUs/{{ $new->image }}" width="80px"></td>
        <td>
          <a href="{{ route('admin.aboutUsLibraryImage.edit',$new->id)}}">
            <i class="material-icons">edit</i>
          </a>
        </td>
        <td>
          <form action="{{ route('admin.aboutUsLibraryImage.destroy', $new->id)}}" method="post">
            @csrf
            @method('DELETE')
...
<div align="right" style="position: fixed;bottom: 30px;right: 30px;">
  <a href="{{ route('admin.aboutUsLibraryImage.create')}}">

...
<div class="card uper">
  ...
  <div class="card-body">
    ...
    @endif
      <form method="post" action="{{ route('admin.studioEquipmentSoftwareImage.update', $studioEquipmentSoftwareImage->id) }}" enctype="multipart/form-data">
        @method('PATCH')
        @csrf
        ...
        <div class="form-group input-group">
          ...
          <div class="row">
            <div class="col-md-3" id="img_contain">
              <img id="previewImage" align='middle' src="{{env('image_url')}}/studioEquipment/{{$studioEquipmentSoftwareImage->image}}" width="200px"  class="pt-3"/>
            </div>
          </div>
          <input type="hidden" name="hidden_image" value="{{ $studioEquipmentSoftwareImage->image }}">
        </div>

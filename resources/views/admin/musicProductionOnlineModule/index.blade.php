...
<div class="table-responsive px-3 pb-5">
 <table class="table table-striped">
    ...
    <tbody>
      @foreach($musicProductionOnlineModule as $key => $row)
        <tr>
            <td>{{ $key+1 }}.</td>
            ...
            <td>
              <a href="{{ route('admin.musicProductionOnlineModule.edit',$row->id)}}">
                Edit
              </a>
            </td>
            <td>
              <form action="{{ route('admin.musicProductionOnlineModule.destroy', $row->id)}}" method="post">
                @csrf
                @method('DELETE')
...
<div align="right" style="position: fixed;bottom: 30px;right: 30px;">
  <a href="{{ route('admin.musicProductionOnlineModule.create')}}">

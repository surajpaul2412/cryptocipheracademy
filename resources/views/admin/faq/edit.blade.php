    ...
    @endif
      <form method="post" action="{{ route('admin.faq.update', $faq->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" name="heading" value="{{ $faq->heading }}"/>
        </div>
        <div class="form-group">
          <label class="text-dark" for="content">Content :</label>
          <textarea id="summernote" class="form-control" name="content">{{ $faq->content }}</textarea>
        </div>
        ...
        <div class="form-group input-group">
          ...
          <div class="row">
            <div class="col-md-3" id="img_contain">
              <img id="previewImage" align='middle' src="{{ URL('/') }}/images/faq/{{ $faq->image }}" width="100px"  class="pt-3"/>
            </div>
          </div>
          <input type="hidden" name="hidden_image" value="{{ $faq->image }}">
        </div>

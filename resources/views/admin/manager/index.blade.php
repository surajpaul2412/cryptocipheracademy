...
<div class="row clearfix pt-5">
    @foreach($users as $user)
      <div class="col-lg-3 mb-4 col-md-3 col-sm-6 col-xs-12">
        <form action="{{ route('admin.manager.destroy', $user->id)}}" method="post">
          @csrf
          @method('DELETE')
        ...
        <a href="{{ route('admin.manager.edit',$user->id)}}" class="edit"><img src="{{ asset('assets/backend/images/writing.png') }}"></a>
      </div>
    @endforeach
</div>
@else
  <h3 class="px-3 pt-3 pb-2 text-dark" align="center">No managers exist please add some.</h3>
@endif


<div align="right" style="position: absolute;bottom: 30px;right: 30px;">
  <a href="{{ route('admin.manager.create')}}">
    <button class="btn px-5 pt-3" style="background: #1d1b27;color:#fff;">Add new manager 

...
<div class="table-responsive px-3 pb-5">
 <table class="table table-striped">
    ...
    <tbody>
      @foreach($studioEquipmentSoftware as $index => $row)
      <tr>
        <th>{{$index+1}}.</th>
        ...
        <td>{{ $row->option5 }}</td>
        <td>
          <a href="{{ route('admin.studioEquipmentSoftware.edit',$row->id)}}">
            <i class="material-icons">edit</i>
          </a>
        </td>
        <td>
          <form action="{{ route('admin.studioEquipmentSoftware.destroy', $row->id)}}" method="post">
            @csrf
            @method('DELETE')
...
<div align="right" style="position: fixed;bottom: 30px;right: 30px;">
  <a href="{{ route('admin.studioEquipmentSoftware.create')}}">

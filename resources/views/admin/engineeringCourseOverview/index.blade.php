...
<div class="table-responsive px-3 pb-5">
 <table class="table table-striped">
    ...
    <tbody>
      @foreach($engineeringCourseOverview as $key => $row)
        <tr>
            <td>{{ $key+1 }}.</td>
            <td class="bold">{!!$row->content!!}</td>
            <td>
              <a href="{{ route('admin.engineeringCourseOverview.edit',$row->id)}}">
                Edit
              </a>
            </td>
            <td>
              <form action="{{ route('admin.engineeringCourseOverview.destroy', $row->id)}}" method="post">
                @csrf
                @method('DELETE')
...
<div align="right" style="position: fixed;bottom: 30px;right: 30px;">
  <a href="{{ route('admin.engineeringCourseOverview.create')}}">

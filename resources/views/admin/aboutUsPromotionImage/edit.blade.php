...
<div class="card uper">
  ...
  <div class="card-body">
    ...
    @endif
      <form method="post" action="{{ route('admin.aboutUsPromotionImage.update', $aboutUsPromotionImage->id) }}" enctype="multipart/form-data">
        @method('PATCH')
        @csrf

        <div class="form-group">
          <label class="text-dark" for="url">Url :</label>
          <input type="text" class="form-control" name="url" value="{{ $aboutUsPromotionImage->url }}"/>
        </div>
        ...
        <div class="form-group input-group">
          ...
          <div class="row">
            <div class="col-md-3" id="img_contain">
              <img id="previewImage" align='middle' src="{{env('image_url')}}/aboutUs/{{ $aboutUsPromotionImage->image }}" width="200px"  class="pt-3"/>
            </div>
          </div>
          <input type="hidden" name="hidden_image" value="{{ $aboutUsPromotionImage->image }}">
        </div>

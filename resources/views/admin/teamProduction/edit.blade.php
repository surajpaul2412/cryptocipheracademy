...
<div class="card uper">
  ...
  <div class="card-body">
    ...
    @endif
      <form method="post" action="{{ route('admin.teamProduction.update', $teamProduction->id) }}" enctype="multipart/form-data">
        @method('PATCH')
        @csrf
        <div class="form-group">
          <label class="text-dark" for="heading">Heading :</label>
          <input type="text" class="form-control" name="heading" value="{{ $teamProduction->heading }}"/>
        </div>

        <div class="form-group">
          <label class="text-dark" for="content">Description :</label>
          <textarea id="summernote" class="form-control" name="content">{{ $teamProduction->content }}</textarea>
        </div>
        ...
        <div class="form-group input-group">
          ...
          <div class="row">
            <div class="col-md-3" id="img_contain">
              <img id="previewImage" align='middle' src="{{env('image_url')}}/team/{{$teamProduction->image}}" width="200px"  class="pt-3"/>
            </div>
          </div>
          <input type="hidden" name="hidden_image" value="{{ $teamProduction->image }}">
        </div>
